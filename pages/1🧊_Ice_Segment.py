...
df_data = load_data("ice")
df_sku_data = load_ref_sku("ice")

# Map of cities and metadata
city_metadata = {
    "SP": ("São Paulo", "Sorvetes Kibon - São Paulo I"),
}
...
for tab, (code, (city_name, store_name)) in zip(tabs, city_metadata.items()):
    df_city_raw = df_data[df_data["crawler_id"] == f"ice-ifood-{code.lower()}-1"]
